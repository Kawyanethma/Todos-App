...
exports.getAllTodosController = async (req, res) => {
  try {
    const todos = await pool.query("SELECT * FROM tasks ORDER BY id DESC");
...
exports.getTodosController = async (req, res) => {
  try {
    const todos = await pool.query(
      "SELECT * FROM tasks WHERE completed = FALSE ORDER BY id DESC LIMIT 5"
    );
    res.status(200).json(todos.rows);
  } catch (err) {
...
exports.addTodoController = async (req, res) => {
  ...
  try {
    const newTodo = await pool.query(
      "INSERT INTO tasks (title, completed, description) VALUES ($1, $2, $3) RETURNING *",
